...
/**
 * Renders the per-entry Open Graph card.
 *
 * This is the theme's original post-card layout, lifted out of
 * `src/pages/posts/[...slug]/index.png.ts` so posts and projects can share one
 * implementation. Adding a third content type should mean adding a route, not
 * copying two hundred lines of Satori tree.
 */
type OgImageInput = {
  /** Headline on the card. */
  title: string;
  /** Rendered as "by {author}" in the footer. */
  author: string;
  /** The request URL, needed to resolve the bundled font files. */
  url: URL;
};
